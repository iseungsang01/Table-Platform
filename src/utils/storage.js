...
export const STORAGE_KEYS = {
  ...
  // 카드 선택 (방문 ID를 키로 사용)
  SELECTED_CARDS: 'selected_cards', // { visitId: cardName }
...
export const storage = {
   ...
   */
  async saveSelectedCard(visitId, cardName) {
    try {
   ...
   * 선택한 카드 조회
   * @param {number} visitId - 방문 기록 ID
   * @returns {string|null} 카드 이름
   */
  async getSelectedCard(visitId) {
    try {
      const cards = await this.get(STORAGE_KEYS.SELECTED_CARDS) || {};
      return cards[visitId] || null;
    } catch (error) {
      console.error('Get selected card error:', error);
      return null;
    }
  },
  ...
  /**
   * 모든 선택한 카드 조회
   * @returns {object} { visitId: cardName }
   */
  async getAllSelectedCards() {
    try {
      return await this.get(STORAGE_KEYS.SELECTED_CARDS) || {};
    } catch (error) {
      console.error('Get all selected cards error:', error);
      return {};
    }
  },
   ...
   * @param {number} visitId - 방문 기록 ID
   */
  async deleteSelectedCard(visitId) {
    try {
      const cards = await this.get(STORAGE_KEYS.SELECTED_CARDS) || {};
  ...
  async cleanupOrphanedCards(validVisitIds) {
    try {
      const cards = await this.getAllSelectedCards();
      const cleanedCards = {};

      Object.keys(cards).forEach(visitId => {
        if (validVisitIds.includes(parseInt(visitId))) {
          cleanedCards[visitId] = cards[visitId];
        }
      });
