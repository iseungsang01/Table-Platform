...
export const visitService = {
  ...
  async getVisits(customerId) {
    try {
      ...
      // 4. 캐시 저장
      await storage.cacheVisits(visitsWithReviews);

      return { data: visitsWithReviews, error: null };
    } catch (error) {
  ...
  async updateVisit(visitId, updates) {
    try {
      // card_review는 로컬에만 저장
      const { card_review, ...serverUpdates } = updates;

      // 1. 리뷰가 있으면 로컬 스토리지에 저장
      ...
      // 리뷰만 업데이트한 경우
      return { data: { id: visitId, card_review }, error: null };
    } catch (error) {
  ...
  /**
   * 로컬 리뷰 데이터 정리 (삭제된 방문 기록의 리뷰 제거)
   * @param {number} customerId - 고객 ID
   */
  async cleanupLocalReviews(customerId) {
    try {
      ...
      console.error('Cleanup local reviews error:', error);
    }
  },
};